...
async function decrypt(secret: string, payloadB64: string): Promise<string> {
  const key = await getAesKey(secret);
...
export type ByokStore = Record<string, string>; // provider -> encrypted(base64)

function requireEncryptionKey(env: Env): string {
  const key =
    ((env?.BYOK_ENCRYPTION_KEY as string | undefined) ?? process.env.BYOK_ENCRYPTION_KEY) || '';
  if (!key || key.length < 16) {
    throw new Error(
      'Missing BYOK_ENCRYPTION_KEY (>=16 chars) in environment. Set it to a strong random value.',
    );
  }
  return key;
}
...
export async function readByokStore(request: Request): Promise<ByokStore> {
  ...
  try {
    ...
    if (typeof parsed === 'string') {
      ...
      return JSON.parse(parsed) as ByokStore;
    }
    return parsed as ByokStore;
  } catch {
    return {};
  }
}

export async function getProviderMasked(
  request: Request,
  env: Env,
  provider: string,
): Promise<{ isSet: boolean; masked?: string }> {
  const store = await readByokStore(request);
  const encrypted = store[provider];
  if (!encrypted) return { isSet: false };
  try {
...
export async function setProviderKey(
  ...
  provider: string,
  rawKey: string,
): Promise<{ headers: Headers; masked: string }> {
  const key = requireEncryptionKey(env);
...
export async function deleteProviderKey(
  ...
  if (Object.keys(store).length === 0) {
    headers.append(
      'Set-Cookie',
      await byokCookie.serialize('', { maxAge: 0 }),
    );
  } else {
    headers.append('Set-Cookie', await byokCookie.serialize(store));
  }
